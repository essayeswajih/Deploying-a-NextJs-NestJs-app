...
export async function GET(request: NextRequest) {
  try {
    ...
    // Simuler les données pour achievements
    const mockData = generateMockData('achievements');
    
    console.log('✅ Données simulées retournées pour achievements:', mockData.length || 'N/A');
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('❌ Erreur création achievements:', error);
    return NextResponse.json(
