...
export async function GET(request: NextRequest) {
  try {
    ...
    // Simuler les données pour resources
    const mockData = generateMockData('resources');
    
    console.log('✅ Données simulées retournées pour resources:', mockData.length || 'N/A');
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('❌ Erreur création resources:', error);
    return NextResponse.json(
