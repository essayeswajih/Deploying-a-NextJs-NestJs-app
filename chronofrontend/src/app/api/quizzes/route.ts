...
export async function GET(request: NextRequest) {
  try {
    ...
    // Simuler les données pour quizzes
    const mockData = generateMockData('quizzes');
    
    console.log('✅ Données simulées retournées pour quizzes:', mockData.length || 'N/A');
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('❌ Erreur création quizzes:', error);
    return NextResponse.json(
