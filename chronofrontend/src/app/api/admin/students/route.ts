...
export async function GET(request: NextRequest) {
  try {
    ...
    // Simuler les données pour students
    const mockData = generateMockData('students');
    
    console.log('✅ Données simulées retournées pour students:', mockData.length || 'N/A');
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('❌ Erreur création students:', error);
    return NextResponse.json(
