...
export async function GET(request: NextRequest) {
  try {
    ...
    // Simuler les données pour parents
    const mockData = generateMockData('parents');
    
    console.log('✅ Données simulées retournées pour parents:', mockData.length || 'N/A');
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('❌ Erreur création parents:', error);
    return NextResponse.json(
